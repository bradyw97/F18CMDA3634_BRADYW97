...
scene_t *sceneSetup(){
  ...
  int Ndisks = 0;    // 1 random disk
  int Nrectangles = 0;// 1  ground plane rectangle

  int Nshapes = Nspheres + Nbunny*Ntriangles + Ncones + 3*Ncylinders + Ndisks + Nrectangles; // each cylinder has two end disks
  ...
  for(i=0;i<Ndisks;++i){
    ...
    shapes[cnt].disk.center.x = 1;
    shapes[cnt].disk.center.y = 1;
    shapes[cnt].disk.center.z = 1;
