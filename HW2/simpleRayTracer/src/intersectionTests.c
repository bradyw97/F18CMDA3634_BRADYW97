...
bool intersectRayDisk(const ray_t r, const disk_t disk, dfloat *pt_t){
  ...
  double num = dot_product(obj.normal,obj.center) - dot_product(obj.normal,observer.start);
  double dum = dot_product(obj.normal,observer.dir);
  dfloat t = 1e9;
  
  // A. find if there is an intersection between ray and disk
  //    i.e. exists point p on disk such that p = s+t*d for some t>=0

  vector_t test_pt = scaled_sum(1.,s,num/dum,d);

  if (d(test_pt,c) > disk.radius) return false;
...
bool intersectRayCylinder(const ray_t r, const cylinder_t cylinder, dfloat *pt_t){
  ...
  h_term = scaled_difference(1.,di, \
			     dot_product(di,ax),ax);
  const_term = scaled_difference(1., difference(s, ce), \
  ...
  if (-1*b + sqrt(fabs(b*b - 4*a*c)) < t) return true;

  if (-1*b > sqrt(b*b - 4*a*c)) {
    ray_dist = (-1*sqrt(b*b - 4*a*c) - b)/(2*a);
    
    test_pt = scaled_sum(1,s,	\
			 ray_dist,di);
