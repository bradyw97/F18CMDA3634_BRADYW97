...
// decorate with underscores bc its a kernel
__global__ void mandelbrot(int Nre, int Nim, complex_t cmin, complex_t dc, float *count){ 
  ...
  // replace something with int n = and int m =
  ...
  int m = m_x + m_x*blocksize_x;
  int n = n_x + n_y*blocksize_y;
...
int main(int argc, char **argv){
  ...
  float *dim_count;
  cudaMalloc(&dim_count,Nre*Nim*sizeof(float));

  dim3 TPB(16,16,1);
  dim3 BPG((Nre+15)/16, Nim/16, 1);
  ...
  // add chevrons containing grid size
  // his hint: multidementional thread block, since its 2D plane for Mandy
  // dim3 TPB(16,16,1); uses C++ so you ccan do 16x16
  // dim3 BPG((Nre+15)/16, Nim/16,1); set of threads org into 16 by 16,
  // mandelbrot<<<BPG,TPB>>>(...);
  ...
  cudaMemcpy(dim_count, count, Nre*Nim*sizeof(float), cudaMemcpyDeviceToHost);
